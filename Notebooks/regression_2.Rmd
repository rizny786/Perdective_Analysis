...
-   Hold last observation for forcasting
...
```{r}
error <- abs(actual - cbind(frc1,frc2ud))
MAE <- colMeans(error)
MAE
...
```{r}
c(MAE, frc3ud = mean(abs(y.tst-frc3ud)),fr4 =mean(abs(y.tst-frc4)))
```

::: {style="color: green"}
OLS model(Trend difference model) is still the better.
:::
